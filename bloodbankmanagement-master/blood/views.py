...
@login_required(login_url='adminlogin')
def admin_hospital_detail_view(request, pk):
    hospital = models.Hospital.objects.get(id=pk)
    return render(request, 'blood/admin_hospital_detail.html', {'hospital': hospital})

# View Hospital Blood Requests
@login_required(login_url='adminlogin')
def admin_hospital_requests_view(request):
    requests = models.BloodRequest.objects.filter(request_by_hospital__isnull=False)
    return render(request, 'blood/admin_hospital_requests.html', {'requests': requests})